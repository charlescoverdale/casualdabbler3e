...
## Current data with readaec

One limitation of `eechidna` is that it bundles data as frozen snapshots — it was last updated for the 2019 election. The [readaec](https://github.com/charlescoverdale/readaec) package takes a different approach: it downloads data directly from the AEC on demand and caches it locally, so 2025 results are available immediately.

```{r, warning=FALSE, message=FALSE}
# Install if needed: remotes::install_github("charlescoverdale/readaec")
...
```{r, warning=FALSE, message=FALSE}
# Who won each seat in 2025?
members_2025 <- get_members_elected(2025)
head(members_2025, 10)
```

```{r, warning=FALSE, message=FALSE}
# TPP results for 2025 — how did each division split between ALP and LNP?
...
```{r, warning=FALSE, message=FALSE}
# Which seats swung most between 2022 and 2025?
get_swing(2022, 2025) |>
  select(division, state, alp_pct_from, alp_pct_to, alp_swing, winner_from, winner_to, seat_changed) |>
  head(15)
```
...
Earlier in this chapter we looked at Richmond's 2022 first preference result — the Greens preferences were decisive. With readaec we can extend that view across every election since 2007 to see the full story.
...
```{r, warning=FALSE, message=FALSE}
# How did Richmond's swing compare to the rest of NSW in 2022?
get_swing(2019, 2022, state = "NSW") |>
  mutate(highlight = tolower(division) == "richmond") |>
  arrange(alp_swing) |>
  mutate(division = factor(division, levels = division)) |>
  ggplot(aes(x = division, y = alp_swing, fill = highlight)) +
  geom_col() +
  geom_hline(yintercept = 0, colour = "grey30") +
  coord_flip() +
  scale_fill_manual(values = c("FALSE" = "grey75", "TRUE" = "#E4281B"), guide = "none") +
  labs(
    title   = "NSW divisions: ALP swing 2019 to 2022",
    subtitle = "Richmond highlighted in red",
    x       = NULL,
...
The AEC publishes polling place locations (lat/lon coordinates) and vote counts at the booth level for every federal election. The `readaec` package makes these easy to access — no manual CSV downloads required.

```{r message=FALSE, error = TRUE, warning=FALSE, echo=TRUE, tidy=TRUE}
# Polling places for the 2025 election (with lat/lon coordinates)
...
ggplot() +
  ...
  labs(
    ...
    subtitle = " ",
    caption = "Data: Australian Electoral Commission 2025 via readaec",
    x = "",
    y = ""
  ) +
  theme_minimal() +
